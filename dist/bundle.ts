...
const node_http_1 = __importDefault(__webpack_require__(67));
const dotenv_1 = __importDefault(__webpack_require__(998));
const userController_1 = __webpack_require__(259);
dotenv_1.default.config();
const PORT = Number(process.env.PORT) || 4000;
const server = node_http_1.default.createServer(userController_1.userController);
...
const node_crypto_1 = __webpack_require__(598);
const db_1 = __webpack_require__(201);
const uuidValidator_1 = __webpack_require__(481);
const httpError_1 = __webpack_require__(240);
exports.userService = {
    getAll() {
        return db_1.users;
    ...
    getByUserId(id) {
        if (!(0, uuidValidator_1.isUUID)(id))
            throw new httpError_1.HttpError(400, 'Invalid UUID');
        const user = db_1.users.find((el) => el.id === id);
        if (!user)
    ...
    create(data) {
        ...
        if (!username || age == null || !Array.isArray(hobbies)) {
            throw new httpError_1.HttpError(400, 'Missing required fields');
        }
        ;
        const newUser = { id: (0, node_crypto_1.randomUUID)(), username, age, hobbies };
    ...
    remove(id) {
        this.getByUserId(id);
        const userIdx = db_1.users.findIndex((el) => el.id === id);
        db_1.users.splice(userIdx, 1);
    }
};
